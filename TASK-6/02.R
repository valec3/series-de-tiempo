...
data <- read_excel("F:\\777--Programacion repos\\Una\\r\\data\\actividad-06.xlsx",sheet = "02")
View(data)
# Gráfica de la serie
data_ts <- ts(data$Producción, start = c(1961,1), frequency = 1)
plot(data_ts, xlab="Años", ylab="Produccion de papa")
...
plot1 <- ggsubseriesplot(data_ts, xlab = "Años", ylab = "Produccion de papa")
plot2 <- ggseasonplot(data_ts, xlab = "Años", ylab = "Produccion de papa")
plot3 <- ggAcf(data_ts, xlab = "Años", ylab = "Produccion de papa")
...
plot(data_diff, xlab="Años", ylab="Produccion de papa")
...
mod1 <- Arima(data_tf, order = c(1, 1, 0), include.constant = T)
coeftest(mod1)

mod2 <- Arima(data_tf, order = c(0, 1, 1), include.constant = T)
coeftest(mod2)

mod3 <- Arima(data_tf, order = c(1, 1, 1), include.constant = T)
coeftest(mod3)
...
Chow_mod1 <- Fstats(mod1$fitted ~ 1, from = 0.45,to=0.55)
sctest(Chow_mod1)

Chow_mod2 <- Fstats(mod2$fitted ~ 1, from = 0.45,to=0.55)
sctest(Chow_mod2)

Chow_mod3 <- Fstats(mod3$fitted ~ 1, from = 0.45,to=0.55)
sctest(Chow_mod3)
...
plot(Pron1, shaded = FALSE, xlab = "Años", ylab = "Produccion de papa",main = "ARIMA(1,1,0)")
...
plot(Pron2, shaded = FALSE, xlab = "Años", ylab = "Produccion de papa",main = "ARIMA(0,1,1)")
...
plot(Pron3, shaded = FALSE, xlab = "Años", ylab = "Produccion de papa",main = "ARIMA(1,1,1)")
...
#GRÁFICA DEL AJUSTE Y PRONÓSTICO CON VALORES REALES
plot(Pron3, shaded = FALSE, xlab = "Años", ylab = "Produccion de papa",main = "ARIMA(1,1,2)")
lines(Pron3$fitted, col = "red")
legend("topleft", legend=c("SERIE", "PREDICCION", "INTERVALO DE COINFIANZA AL 95%", "AJUSTE"),col=c("black", "blue", "black", "red"), lty=c(1,1,2,1), lwd = 2,cex = 0.6)
abline(v=2006.7, lwd = 1, col="green")
